...
# متطلبات قواعد البيانات لنظام NetPulse (Database Requirements Specification)

تعتمد البنية التحتية للبيانات في النظام على استراتيجية فصل المجالات وتوزيعها بين قاعدتي بيانات رئيسيتين (Oracle و MySQL) لضمان عدم تكرار البيانات التشغيلية وتحديد مصادر الحقائق بدقة.

### أولاً: متطلبات قاعدة بيانات أوراكل (Oracle Database Requirements - المجال التشغيلي)

تُعد قاعدة بيانات أوراكل المصدر الأساسي والوحيد للحقائق المتعلقة بالأجهزة، الأحداث، التنبيهات، والحوادث. ويجب أن تلبي المتطلبات الوظيفية التالية:


1. **إدارة بيانات الأجهزة (Device Inventory Management):**
 
    - يجب أن تدعم قاعدة البيانات تخزين وتتبع السجلات التعريفية والتشغيلية لكل جهاز في الشبكة، متضمنةً اسم الجهاز، نوعه، عنوان بروتوكول الإنترنت (IP)، دوره في الشبكة (مثل رئيسي أو وصول)، حالته التشغيلية الحالية، وطوابع زمنية لإنشاء وتحديث السجلات.
        
2. **تسجيل أحداث القياس عن بعد (Telemetry Events Management):**

    - يجب أن تسجل قاعدة البيانات الأحداث الخام والواردة من الأجهزة مع ربط كل حدث بمعرف الجهاز المصدر، وتخزين نوع الحدث، مستوى الخطورة، والحمولة البيانية للحدث (Payload) بتنسيق CLOB، إلى جانب طوابع زمنية دقيقة لوقت الاستلام والمعالجة. 
        
3. **إدارة قواعد التنبيه (Alert Rules Management):**

    - يجب أن تتيح تخزين قواعد التنبيه المبسطة التي تحدد معايير إطلاق التنبيه بناءً على نوع الحدث ودور الجهاز ومستوى الخطورة، مع دعم خاصية تفعيل أو تعطيل القاعدة (Enabled/Disabled).
        
4. **إدارة التنبيهات (Alerts Management):**
    - يجب أن تسجل قاعدة البيانات التنبيهات الناتجة عن مطابقة أحداث القياس مع قواعد التنبيه، مع الاحتفاظ برسالة التنبيه، مستوى خطورته، حالته، وتواريخ الاكتشاف والإقرار والحل.

5. **إدارة الحوادث التشغيلية (Incident Management):** 
    - يجب أن تدعم إنشاء وتخزين الحوادث الناشئة عن التنبيهات الحرجة، متضمنةً عنوان الحادث، الوصف التفصيلي، الأولوية، حالة الحادث (المضمنة لحالات: مفتوح، قيد المعالجة، محلولة، مغلقة)، معرف المستخدم المُسند إليه الحادث، وتواريخ الإنشاء والحل والإغلاق.

6. **إدارة حسابات النظام التشغيلي (System User Accounts):**
 
    - يجب أن تخزن بيانات حسابات المستخدمين المخولين للوصول إلى النظام التشغيلي، بما في ذلك أسماء المستخدمين الفريدة، كلمات المرور المشفرة (Password Hashes)، والأسماء الكاملة، والأدوار الصلاحية المعتمدة (ADMIN, ENGINEER, VIEWER).

7. **سجلات التدقيق الخلفي (Backend Audit Logging):**
    
      
    - يجب أن توفر سجلاً تدقيقياً لتتبع العمليات الهامة والإجراءات المنفذة داخل النظام التشغيلي، مسجلةً هوية المستخدم، نوع الإجراء، نوع الكيان المستهدف ومعرفه، ووصفاً نصياً للتغيير مع طابع زمني.

### ثانياً: متطلبات قاعدة بيانات MySQL (MySQL Database Requirements - مجال التذاكر)

تُعد قاعدة بيانات MySQL المصدر الأساسي لإدارة دورة حياة تذاكر الدعم الفني والبوابة الخاصة بها. ويجب أن تلبي المتطلبات الوظيفية التالية:

1. **إدارة تذاكر الدعم الفني (Ticketing Management):**
    
      
    - يجب أن تدعم تخزين بيانات التذاكر المرتبطة بالحوادث الواردة من النظام التشغيلي، متضمنةً رقم التذكرة النصي الفريد، معرف الحادث المرتبط، عنوان التذكرة، الوصف، الأولوية، الحالة، المُعرّف المُسند إليه التذكرة، وتواريخ الإنشاء والتحديث والإغلاق.

2. **إدارة مستخدمي بوابة الويب (Web Portal User Management):**
    - يجب أن تدعم تخزين حسابات ومصادقة المستخدمين الخاصين ببوابة الويب للتعامل مع التذاكر، متضمنةً أسماء المستخدمين، كلمات المرور المشفرة، الصلاحيات، وحالة الحساب.

3. **إدارة تعليقات التذاكر (Ticket Comments Management):**
    - يجب أن تسجل وتخزن التعليقات والملاحظات النصية المضافة على التذاكر من قبل المهندسين والمستخدمين مع ربطها بهوية صاحب التعليق وطابع زمني.  
        
4. **تتبع سجل تاريخ التذاكر (Ticket History Tracking):**
    - يجب أن تحتفظ بسجل زمني ومفصل لكل تغيير يطرأ على حالة التذكرة أو مسارها، مسجلةً الحالة القديمة والحالة الجديدة، والمستخدم الذي أحدث التغيير، والتعليق المرتبط بالتعديل لأغراض المراجعة والتدقيق.
### ثالثاً: متطلبات تكامل وفصل قواعد البيانات (Integration & Domain Separation Requirements)

1. **منع ازدواجية البيانات (No Data Duplication):**
    - يجب ألا تقوم أي من القاعدتين بتكرار أو إنشاء نسخ منافسة للسجلات التابعة للمجال الآخر؛ فأوراكل مسؤولة عن البنية التشغيلية والأحداث، بينما MySQL مسؤولة عن التذاكر والتعليقات الخاصة بها.
        
2. **الربط المرجعي المشترك (Shared References):**  
    - يجب أن يعتمد التكامل بين النظامين على مشاركة معرّفات مرجعية خفيفة الوزن فقط (مثل ربط تذاكر MySQL بحوادث أوراكل عبر حقل `incident_id` ورقم التذكرة `ticket_number`) دون الحاجة لدمج قواعد البيانات أو إنشاء تعقيدات هيكلية غير ضرورية في مرحلة الـ MVP.
